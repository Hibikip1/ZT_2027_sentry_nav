...
struct integer_traits
{
  ...
  static constexpr auto _max = [] {
    if constexpr(std::is_same_v<T, char>)
      ...
      static_assert(_detail::error<T>, "specialize integer_traits for your custom "
                                       "integer types");
  }();
  template <int Radix>
...
struct integer_traits<bounded<T, Max>>
{
  ...
  static constexpr bool add_digit_checked(type& result, unsigned digit)
  {
    return integer_traits<T>::template add_digit_checked<Radix>(result, digit) &&
           result <= Max;
  }
};
}  // namespace lexy
...
struct _bounded_integer_parser
{
  ...
  static_assert(max_digit_count > 1, "integer must be able to store all possible digit "
                                     "values");
...
{
  using value_type = std::conditional_t<_ndigits_can_overflow<T, N, Base::digit_radix>(),
                                        T, lexy::unbounded<T>>;
...
constexpr auto code_point_id = [] {
  using type =
      std::conditional_t<_ndigits_can_overflow<lexy::code_point, N, Base::digit_radix>(),
...
constexpr auto code_unit_id = [] {
  ...
  using type =
      std::conditional_t<_ndigits_can_overflow<char_type, N, Base::digit_radix>(),
