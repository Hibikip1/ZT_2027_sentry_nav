  ...
  struct p
  {
    ...
    LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&...)
    {
      ...
      if constexpr(!std::is_same_v<Rule, void>)
      {
        ...
        lexy::do_action<_err_production<Rule>,
                        lexy::match_action<void, Reader>::template result_type>(
            lexy::_mh(), context.control_block->parse_state, reader);
        end = reader.position();
...
struct _must : branch_base
{
  ...
  struct p
  {
    ...
    LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args)
    {
      ...
      auto result =
          lexy::parser_for<Error, lexy::pattern_parser<>>::parse(context, reader);
