...
constexpr auto match_literal(Reader& reader)
{
  ...
  else if constexpr(is_swar_reader<Reader> && sizeof...(Cs) >= swar_length<char_type>)
  {
    ...
    {
      auto partial =
          swar_find_difference<CharT>(reader.peek_swar() & pack.mask, pack.value);
...
struct lit_trie_matcher<Trie, CurNode>
{
  ...
  LEXY_FORCE_INLINE static constexpr bool _try_transition(std::size_t& result,
                                                          Reader& reader, IntT cur)
  {
    ...
    _lit_base
{
  ...
  struct tp
  {
    ...
    constexpr void report_error(Context& context, const Reader& reader)
    {
      ...
      auto err =
          lexy::error<Reader, lexy::expected_literal>(begin, str, index, sizeof...(C));
...
struct _lcp : token_base<_lcp<Cp...>>, _lit_base
{
  ...
  template <typename Reader, typename Indices = lexy::_detail::make_index_sequence<
