...
/* not force inline */ constexpr bool _finish_production(ProductionParser& parser,
                       ...
                       _copy_base<lexy::production_rule<Production>>>
{
  ...
  struct p
  {
    ...
    LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args)
    {
      ...
      {
        if(!lexy::whitespace_parser<decltype(sub_context), lexy::pattern_parser<>>::parse(
               sub_context, reader))
        {
...
struct _recb : branch_base
{
  ...
  struct _depth_handler
  {
    ...
    static constexpr bool increment_depth(Context& context, Reader& reader)
    {
      ...
      LEXY_ASSERT(control_block->max_depth > 0, "dsl::recurse_branch<P> is disabled in "
                                                "this context");
      ...
      if(control_block->cur_depth >= control_block->max_depth)
      {
        ...
        using tag =
            lexy::_detail::type_or<DepthError, lexy::max_recursion_depth_exceeded>;
