...
using _detect_op_tag_ctor_with_state = decltype(Tag(
    *LEXY_DECLVAL(Context).control_block->parse_state, LEXY_DECLVAL(Reader).position()));
...
struct _op : branch_base
{
                                         ...
                                         Args&&... args)
  {
    ...
    else if constexpr(lexy::_detail::is_detected<_detect_op_tag_ctor, op_tag_type,
                                                 Reader>)
      return continuation::parse(context, reader, LEXY_FWD(args)...,
...
{
  static_assert(lexy::is_literal_rule<Literal>, "condition in the operator must be a "
                                                "literal rule");
...
struct _opc : branch_base
{
  ...
  struct p
  {
    ...
    LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args)
    {
      ...
      {
        auto err =
            lexy::error<Reader, lexy::expected_literal_set>(impl.op.cur.position());
