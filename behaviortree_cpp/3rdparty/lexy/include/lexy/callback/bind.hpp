...
_invoke_bound(Fn&& fn, const _detail::tuple<BoundArgs...>& bound_args, State& state,
              const _detail::tuple<ActualArgs...>& actual_args,
              _detail::index_sequence<ActualIdx...>, ProducedArgs&&... produced_args)
{
  ...
  else
  {
    ...
    else
    {
      return _invoke_bound<Idx + 1>(
          LEXY_FWD(fn), bound_args, state, actual_args, actual_args.index_sequence(),
          LEXY_FWD(produced_args)...,
...
struct _parse_state : _detail::placeholder_base
{
  ...
  {
    static_assert(!std::is_same_v<State, _detail::no_bind_state>, "lexy::parse_state "
...
struct _bound_sink
{
  ...
  template <bool Dummy = true, typename = std::enable_if_t<
                                   (!_detail::is_placeholder<BoundArgs> && ... && Dummy)>>
...
constexpr auto bind_sink(Sink&& sink, BoundArgs&&... args)
{
  static_assert(
      ...
      "lexy::values as a placeholder for bind_sink() doesn't make sense - there won't be "
      "any values");
