...
public:
  ...
  {
  public:
    explicit builder(std::size_t size, MemoryResource* resource =
  ...
  static buffer
  adopt(const char_type* data, std::size_t size,
  ...
  explicit buffer(const View& view, MemoryResource* resource =
                                        _detail::get_memory_resource<MemoryResource>())
...
struct _make_buffer
{
  ...
  auto operator()(
      const void* _memory, std::size_t size,
...
struct _make_buffer<utf8_char_encoding, encoding_endianness::bom>
{
      ...
      MemoryResource* resource = _detail::get_memory_resource<MemoryResource>()) const
  {
    ...
    return _make_buffer<utf8_char_encoding, encoding_endianness::big>{}(memory, size,
                                                                        resource);
  }
};
template <>
  ...
  else
  {
    ...
    else
    {
      ...
      for(auto cur = begin; cur != end; ++cur)
        *dest++ =
            *cur;  // NOLINT: clang-analyzer thinks this might access zero memory for
                   // some reason?!
      return LEXY_MOV(builder).finish();
    }
  }
