...
class scanner
  : public _detail::scanner<scanner<ControlProduction, Input, State, ErrorCallback>,
...
public:
  constexpr explicit scanner(const Input& input, State* state,
    ...
    : _impl(input.reader())
    , _input(&input)
    , _sink(_get_error_sink(callback))
...
{
  return scanner<ControlProduction, Input, void, ErrorCallback>(input, no_parse_state,
                                                                callback);
}
...
{
  return scanner<ControlProduction, Input, const State, ErrorCallback>(input, &state,
                                                                       callback);
}
}  // namespace lexy
