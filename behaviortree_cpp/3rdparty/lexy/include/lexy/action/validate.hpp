...
constexpr auto _get_error_sink(const Callback& callback)
{
  ...
  {
    static_assert(std::is_void_v<typename Callback::return_type>, "need to use "
                                                                  "`lexy::collect()` to "
                                                                  "create an error "
                                                                  "callback that can "
                                                                  "handle multiple "
...
public:
  ...
  static_assert(!std::is_void_v<error_type>, "ErrorCallback must not be a void returning "
                                             "sink");
...
public:
  ...
  public:
    ...
    {
      handler._cb.char_class(handler._cb.sink, get_info(), handler._cb.input, _begin,
                             error);
    }
...
constexpr auto validate(const Input& input, State& state, const ErrorCallback& callback)
    -> validate_result<ErrorCallback>
{
  return validate_action<State, Input, ErrorCallback>(state, callback)(Production{},
                                                                       input);
}
template <typename Production, typename Input, typename State, typename ErrorCallback>
...
{
  return validate_action<const State, Input, ErrorCallback>(state, callback)(Production{},
                                                                             input);
}
}  // namespace lexy
