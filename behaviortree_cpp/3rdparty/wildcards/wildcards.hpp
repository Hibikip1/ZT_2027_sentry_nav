...
struct cards<char>
{
  ...
  constexpr cards(char a, char s, char e)
    ...
    , alt_enabled{ false }
  {}
  constexpr cards(char a, char s, char e, char so, char sc, char sn, char ao, char ac,
                  char ar)
    : anything{ std::move(a) }
    , single{ std::move(s) }
    , escape{ std::move(e) }
...
struct cards<char16_t>
{
  ...
  constexpr cards(char16_t a, char16_t s, char16_t e)
    ...
    , alt_enabled{ false }
  {}
  constexpr cards(char16_t a, char16_t s, char16_t e, char16_t so, char16_t sc,
...
struct cards<char32_t>
{
  ...
  constexpr cards(char32_t a, char32_t s, char32_t e)
    ...
    , alt_enabled{ false }
  {}
  constexpr cards(char32_t a, char32_t s, char32_t e, char32_t so, char32_t sc,
...
template <typename T>
constexpr cards<T> make_cards(T&& a, T&& s, T&& e, T&& so, T&& sc, T&& sn, T&& ao, T&& ac,
                              T&& ar)
{
  return { std::forward<T>(a),  std::forward<T>(s),  std::forward<T>(e),
...
{
  using type = typename std::remove_cv<typename std::remove_reference<decltype(cx::cbegin(
      std::declval<C>()))>::type>::type;
};
template <typename C>
...
{
  using type = typename std::remove_cv<typename std::remove_reference<decltype(*cx::begin(
      std::declval<C>()))>::type>::type;
};
template <typename C>
...
make_full_match_result(SequenceIterator s, SequenceIterator send, PatternIterator p,
    ...
    switch(state)
    {
      ...
      default:
        ...
        throw std::logic_error("The program execution should never end up here throwing "
                               "this exception");
#else
        return throw_logic_error("The program execution should never end up here "
...
template <typename PatternIterator>
constexpr PatternIterator set_end(PatternIterator p, PatternIterator pend,
                                  const cards<iterated_item_t<PatternIterator>>& c =
                                      cards<iterated_item_t<PatternIterator>>(),
    ...
    switch(state)
    {
      ...
      default:
        ...
        throw std::logic_error("The program execution should never end up here throwing "
                               "this exception");
#else
        return throw_logic_error(p, "The program execution should never end up here "
                                    "throwing this exception");
#endif
    }
    p = cx::next(p);
    ...
    const EqualTo& equal_to = EqualTo(), match_set_state state = match_set_state::open)
{
  ...
  if(!c.set_enabled)
  {
    ...
    throw std::invalid_argument("The use of sets is disabled");
#else
    return throw_invalid_argument(make_match_result(false, s, p), "The use of sets is "
                                                                  "disabled");
#endif
  }
  while(p != pend)
    ...
    switch(state)
    {
      case match_set_state::open:
        if(*p != c.set_open)
        {
          ...
          throw std::invalid_argument("The given pattern is not a valid set");
#else
          return throw_invalid_argument(make_match_result(false, s, p), "The given "
                                                                        "pattern is not "
                                                                        "a valid set");
#endif
        }
      ...
      default:
        ...
        throw std::logic_error("The program execution should never end up here throwing "
                               "this exception");
#else
        return throw_logic_error(make_match_result(false, s, p), "The program execution "
                                                                 "should never end up "
                                                                 "here throwing this "
                                                                 "exception");
#endif
  ...
  throw std::invalid_argument("The given pattern is not a valid set");
#else
  return throw_invalid_argument(make_match_result(false, s, p), "The given pattern is "
                                                                "not a valid set");
#endif
#else
  return !c.set_enabled ? throw std::invalid_argument("The use of sets is disabled") :
         ...
         state == match_set_state::open ?
                     *p == c.set_open ? match_set(s, send, cx::next(p), pend, c, equal_to,
                                                  match_set_state::not_or_first_in) :
                                        throw std::invalid_argument("The given pattern "
                                                                    "is not a valid "
                                                                    "set") :
         state == match_set_state::not_or_first_in ?
                     *p == c.set_not  ? match_set(s, send, cx::next(p), pend, c, equal_to,
...
template <typename PatternIterator>
constexpr PatternIterator alt_end(PatternIterator p, PatternIterator pend,
                                  const cards<iterated_item_t<PatternIterator>>& c =
                                      cards<iterated_item_t<PatternIterator>>(),
...
#else
          return throw_invalid_argument(p, "The given pattern is not a valid "
                                           "alternative");
#endif
        }
...
#endif
#else
  return !c.alt_enabled ? throw std::invalid_argument("The use of alternatives is "
                                                      "disabled") :
         p == pend      ? throw std::invalid_argument("The given pattern is not a valid "
                                                           "alternative") :
         state == alt_end_state::open ?
                     *p == c.alt_open ?
                     alt_end(cx::next(p), pend, c, alt_end_state::next, depth + 1) :
                     throw std::invalid_argument("The given pattern is not a valid "
                                                 "alternative") :
         state == alt_end_state::next ?
                     *p == c.escape ?
                     ...
                     c.set_enabled && *p == c.set_open &&
                             is_set(cx::next(p), pend, c, is_set_state::not_or_first) ?
                     alt_end(set_end(cx::next(p), pend, c, set_end_state::not_or_first),
                             pend, c, state, depth) :
...
template <typename PatternIterator>
constexpr PatternIterator alt_sub_end(PatternIterator p, PatternIterator pend,
                                      const cards<iterated_item_t<PatternIterator>>& c =
                                          cards<iterated_item_t<PatternIterator>>(),
  ...
  return !c.alt_enabled ?
         ...
         state == alt_sub_end_state::next ?
             *p == c.escape ?
             ...
             c.set_enabled && *p == c.set_open &&
                     is_set(cx::next(p), pend, c, is_set_state::not_or_first) ?
             alt_sub_end(set_end(cx::next(p), pend, c, set_end_state::not_or_first), pend,
                         c, state, depth) :
             *p == c.alt_open ?
             ...
             *p == c.alt_or ?
             depth == 1 ? p : alt_sub_end(cx::next(p), pend, c, state, depth) :
...
template <typename SequenceIterator, typename PatternIterator,
          typename EqualTo = cx::equal_to<void>>
constexpr match_result<SequenceIterator, PatternIterator>
match_alt(SequenceIterator s, SequenceIterator send, PatternIterator p1,
          ...
          const EqualTo& equal_to = EqualTo(), bool partial = false)
{
  ...
  return match(s, send, p1, p1end, c, equal_to, true) &&
                 match(match(s, send, p1, p1end, c, equal_to, true).s, send, p2, p2end, c,
                       equal_to, partial) ?
             match(match(s, send, p1, p1end, c, equal_to, true).s, send, p2, p2end, c,
                   equal_to, partial) :
         cx::next(p1end) == p2 ?
...
constexpr match_result<SequenceIterator, PatternIterator>
match(SequenceIterator s, SequenceIterator send, PatternIterator p, PatternIterator pend,
      const cards<iterated_item_t<PatternIterator>>& c, const EqualTo& equal_to,
      bool partial, bool escape)
{
#if cfg_HAS_CONSTEXPR14
  if(p == pend)
  {
     ...
     is_set(cx::next(p), pend, c, is_set_state::not_or_first))
  {
    ...
    return match(cx::next(s), send,
                 set_end(cx::next(p), pend, c, set_end_state::not_or_first), pend, c,
                 equal_to, partial);
  }
     ...
     is_alt(cx::next(p), pend, c, is_alt_state::next, 1))
  {
    ...
    return match_alt(s, send, cx::next(p), alt_sub_end(cx::next(p), p_alt_end, c),
         ...
         c.set_enabled && *p == c.set_open &&
                 is_set(cx::next(p), pend, c, is_set_state::not_or_first) ?
                  ...
                  match(cx::next(s), send,
                        set_end(cx::next(p), pend, c, set_end_state::not_or_first), pend,
                        c, equal_to, partial) :
         c.alt_enabled && *p == c.alt_open &&
                 is_alt(cx::next(p), pend, c, is_alt_state::next, 1) ?
                  match_alt(s, send, cx::next(p),
                            alt_sub_end(
                                cx::next(p),
                                alt_end(cx::next(p), pend, c, alt_end_state::next, 1), c),
                            alt_end(cx::next(p), pend, c, alt_end_state::next, 1), pend,
                            c, equal_to, partial) :
         s == send || !equal_to(*s, *p) ?
                  make_match_result(false, s, p) :
                  match(cx::next(s), send, cx::next(p), pend, c, equal_to, partial);
      ...
      const EqualTo& equal_to = EqualTo())
{
  return detail::make_full_match_result(
      ...
      detail::match(cx::cbegin(sequence), cx::cend(std::forward<Sequence>(sequence)),
                    cx::cbegin(pattern), cx::cend(std::forward<Pattern>(pattern)), c,
                    equal_to));
}
template <
    typename Sequence, typename Pattern, typename EqualTo = cx::equal_to<void>,
...
template <typename T>
constexpr bool operator==(const basic_string_view<T>& lhs,
...
template <
    ...
    typename = typename std::enable_if<!std::is_same<EqualTo, cards_type>::value>::type>
constexpr matcher<Pattern, EqualTo> make_matcher(Pattern&& pattern,
                                                 const EqualTo& equal_to)
{
  return make_matcher(std::forward<Pattern>(pattern), cards<container_item_t<Pattern>>(),
                      equal_to);
}
namespace literals
{
