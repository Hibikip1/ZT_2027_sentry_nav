...
#endif
  event_buffer =
      (raw_event_t*)malloc(INTERNAL_MINITRACE_BUFFER_SIZE * sizeof(raw_event_t));
  flush_buffer =
      (raw_event_t*)malloc(INTERNAL_MINITRACE_BUFFER_SIZE * sizeof(raw_event_t));
  is_tracing = 1;
  event_count = 0;
  ...
  for(i = 0; i < event_count_copy; i++)
  {
    ...
    switch(raw->arg_type)
    {
      ...
      case MTR_ARG_TYPE_STRING_COPY:
        ...
        {
          snprintf(arg_buf, ARRAY_SIZE(arg_buf), "\"%s\":\"%.*s\"", raw->arg_name, 700,
                   raw->a_str);
        }
        else
        {
    ...
    len = snprintf(linebuf, ARRAY_SIZE(linebuf),
                   "%s{\"cat\":\"%s\",\"pid\":%i,\"tid\":%i,\"ts\":%" PRId64 ",\"ph\":\"%"
                   "c\","
                   "\"name\":"
                   "\"%s\","
                   "\"args\":{%"
