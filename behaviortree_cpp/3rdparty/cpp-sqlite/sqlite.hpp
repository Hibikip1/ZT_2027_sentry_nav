...
public:
  ...
  explicit Blob(std::vector<unsigned char> data) : m_data(std::move(data))
  {}
...
{
public:
  NOBlob(const void* ptr, uint32_t bytes) : m_ptr(ptr), m_bytes(bytes)
  {}
...
inline void Append(sqlite3_stmt* statement, int index, const float& data)
{
  sqlite::Priv::CheckError(
      sqlite3_bind_double(statement, index, static_cast<double>(data)));
}
...
{
  sqlite::Priv::CheckError(sqlite3_bind_text(statement, index, data.data(),
                                             static_cast<int>(data.size()), nullptr));
}

inline void Append(sqlite3_stmt* statement, int index, const char* data)
{
  sqlite::Priv::CheckError(sqlite3_bind_text(
      statement, index, data, static_cast<int>(std::strlen(data)), nullptr));
}

inline void Append(sqlite3_stmt* statement, int index, const sqlite::Blob& blob)
{
  sqlite::Priv::CheckError(sqlite3_bind_blob(statement, index, blob.GetData(),
                                             static_cast<int>(blob.GetSize()), nullptr));
}
...
inline void AppendToQuery(sqlite3_stmt* statement, int index, const First& first,
                          const Args&... args)
{
...
struct Statement
{
  ...
  Statement(sqlite::Connection& connection, const std::string& command)
  {
    ...
    const int code = sqlite3_prepare_v2(
        db, command.data(), static_cast<int>(command.size()), &handle, nullptr);
  ...
  std::string GetColumnName(int columnIndex) const
  {
    ...
    {
#ifndef CPP_SQLITE_NOTHROW
      throw sqlite::Error("SQL error: failed to get column name at index " +
...
class Result
{
  explicit Result(sqlite::Priv::Statement&& statement) : m_statement(std::move(statement))
  {}
...
template <typename First, typename... Args>
CPP_SQLITE_NODISCARD inline Result Query(sqlite::Connection& connection,
                                         const std::string& command, const First& first,
                                         const Args... args)
{
  sqlite::Priv::Statement statement(connection, command);
  sqlite::Priv::AppendToQuery<First, Args...>(statement.handle, 1, first, args...);
...
inline bool Backup(sqlite::Connection& from, sqlite::Connection& to)
{
  sqlite3_backup* backup =
      sqlite3_backup_init(to.GetPtr(), "main", from.GetPtr(), "main");
