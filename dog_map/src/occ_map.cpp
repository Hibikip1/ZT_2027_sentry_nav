...
OccMap::OccMap(double heighter_than_ground_threshold,
    ...
    std::string pgm_path = "(none)";
    if (!pgm_yaml_path.empty()) {
        ...
        if (!map_img.empty()) {
            ...
            std::cout << " -- [DOG_MAP] WARN: prior map image not found: "
                      << pgm_path << ", fallback to empty grid." << std::endl;
            initEmptyMap();
        }
    } else {
        std::cout << " -- [DOG_MAP] No prior map (pgm_yaml_path empty), "
                     "use empty grid." << std::endl;
        initEmptyMap();
    }
/*************fix map init */
#ifdef FIX_MAP
...
void OccMap::initEmptyMap() {
    if (resolution <= 0.0) {
        resolution = 0.1;  // 默认栅格分辨率
    }
    if (cloud_at_map_origin.empty()) {
        cloud_at_map_origin = {0.0, 0.0};
    }
    int map_size = static_cast<int>(2.0 * half_map_size_ / resolution);
    cv::Mat empty_map = cv::Mat::zeros(map_size, map_size, CV_8UC1);
    std::cout << " -- [DOG_MAP] Empty grid: " << map_size << "x" << map_size
              << " @ " << resolution << " m/px, layers="
              << static_cast<int>(1.0 / resolution_z) << std::endl;
    InitMap(empty_map);
}
void OccMap::Clear() {
