...
void HeroMpcController::configure(
    ...
    // 初始化 MPC 求解器
    // -------------------------------------------------------------------------
    RCLCPP_INFO(logger_, "[%s] STEP: 参数读取完成, 创建 MpcWrapper", plugin_name_.c_str());
    mpc_wrapper_ = std::make_unique<MpcWrapper>();
    RCLCPP_INFO(logger_, "[%s] STEP: MpcWrapper 创建完成, init()", plugin_name_.c_str());
    if (!mpc_wrapper_->init()) {
        throw nav2_core::PlannerException("MPC 求解器初始化失败!");
    }
    RCLCPP_INFO(logger_, "[%s] STEP: init() 完成", plugin_name_.c_str());
    ...
    // 设置约束
    RCLCPP_INFO(logger_, "[%s] STEP: set_weights 完成", plugin_name_.c_str());
    mpc_wrapper_->set_control_bounds(max_acc_, max_alpha_);
    mpc_wrapper_->set_velocity_bounds(max_vel_, max_omega_);
    RCLCPP_INFO(logger_, "[%s] STEP: set_bounds 完成", plugin_name_.c_str());
    ...
    RCLCPP_INFO(logger_, "[%s] STEP: 创建订阅器/发布器", plugin_name_.c_str());
    minco_traj_sub_ =
        node->create_subscription<interfaces::msg::MincoTrajectory>(
            // 相对话题:在命名空间内解析为 <ns>/smoother_server/minco_polynomial_trajectory,
            // 与 smoother_server 的 ~/minco_polynomial_trajectory 匹配(空命名空间时等价于全局)
    ...
    carrot_pose_pub_ = node->create_publisher<interfaces::msg::NavOutput>(
        "carrot_pose", rclcpp::SensorDataQoS());  // 相对话题,命名空间内与决策层匹配
