...
void FakeVelTransform::cmdVelCallback(const geometry_msgs::msg::Twist::SharedPtr msg)
{
  ...
  if (
    ...
    latest_cmd_vel_ = msg;
    latest_cmd_vel_time_ = rclcpp::Clock().now();
  }
}
  ...
  const nav_msgs::msg::Path::ConstSharedPtr & /*local_plan_msg*/)
{
  ...
  // 标记已消费(更新缓存时刻),避免 0.2s 回退转发与正常同步转发冲突(双重发布)
  latest_cmd_vel_time_ = rclcpp::Clock().now();
}
...
void FakeVelTransform::publishTransform()
{
  ...
  // 用当前时间(保证 controller/costmap 能查到当前时刻的 fake TF);
  // 若当前时间意外早于最新 odometry(时钟跳变),回退到 odometry 时间戳,避免 tf2 jump back
  t.header.stamp = this->get_clock()->now();
  // last_odom_stamp_ 是 ROS 时间(来自 odometry 消息);用 nanoseconds() 判断有效性,
  // 避免与 rclcpp::Time(0)(系统时间)比较时抛 "can't compare times with different time sources"
  if (last_odom_stamp_.nanoseconds() != 0 && rclcpp::Time(t.header.stamp) < last_odom_stamp_) {
    t.header.stamp = last_odom_stamp_;
  }
  ...
  // 鲁棒回退:若 cmd_vel 缓存超过 0.2s 仍未与 odometry/local_plan 同步成功,
  // 直接用当前底盘角度转发,避免 "Failed to make progress"(同步失败导致车完全不动)
  {
    std::lock_guard<std::mutex> lock(cmd_vel_mutex_);
    if (latest_cmd_vel_ && latest_cmd_vel_time_.nanoseconds() != 0 &&
      (rclcpp::Clock().now() - latest_cmd_vel_time_).seconds() > 0.2) {
      geometry_msgs::msg::Twist aft_tf_vel =
        transformVelocity(latest_cmd_vel_, current_robot_base_angle_);
      cmd_vel_chassis_pub_->publish(aft_tf_vel);
    }
  }
}
