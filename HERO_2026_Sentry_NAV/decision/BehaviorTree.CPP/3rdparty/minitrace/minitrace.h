...
MINITRACE_EXPORT void internal_mtr_raw_event(const char* category, const char* name,
                                             char ph, void* id);
MINITRACE_EXPORT void internal_mtr_raw_event_arg(const char* category, const char* name,
                                                 char ph, void* id, mtr_arg_type arg_type,
...
#define MTR_STEP(c, n, id, step)                                                         \
  internal_mtr_raw_event_arg(c, n, 'T', (void*)(id), MTR_ARG_TYPE_STRING_CONST, "step",  \
                             (void*)(step))
...
#define MTR_INSTANT_C(c, n, aname, astrval)                                              \
  internal_mtr_raw_event_arg(c, n, 'I', 0, MTR_ARG_TYPE_STRING_CONST, aname,             \
                             (void*)(astrval))
...
#define MTR_META_PROCESS_NAME(n)                                                         \
  internal_mtr_raw_event_arg("", "process_name", 'M', 0, MTR_ARG_TYPE_STRING_COPY,       \
                             "name", (void*)(n))
#define MTR_META_THREAD_NAME(n)                                                          \
  internal_mtr_raw_event_arg("", "thread_name", 'M', 0, MTR_ARG_TYPE_STRING_COPY,        \
                             "name", (void*)(n))
#define MTR_META_THREAD_SORT_INDEX(i)                                                    \
  internal_mtr_raw_event_arg("", "thread_sort_index", 'M', 0, MTR_ARG_TYPE_INT,          \
                             "sort_index", (void*)(i))
...
#define MTR_SCOPE_FUNC_LIMIT_MS(l)                                                       \
  MTRScopedTraceLimit ____mtr_scope(__FILE__, __FUNCTION__, (double)l * 0.000001)
...
{
public:
  MTRScopedTraceArg(const char* category, const char* name, mtr_arg_type arg_type,
                    const char* arg_name, void* arg_value)
    : category_(category), name_(name)
  {
