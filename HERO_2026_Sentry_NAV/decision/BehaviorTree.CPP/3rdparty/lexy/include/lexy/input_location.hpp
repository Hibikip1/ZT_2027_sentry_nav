...
auto _compute_default_location_counting()
{
  ...
  if constexpr(lexy::is_byte_encoding<encoding>)
    ...
    static_assert(_detail::error<Input>, "input encoding does not have a default "
                                         "location counting policy");
}
...
public:
  constexpr explicit input_location(const Input& input)
    ...
    , _column_begin(_line_begin.position())
    , _line_nr(1)
    , _column_nr(1)
  {}
...
private:
  ...
  get_input_location(const I& input, typename lexy::input_reader<I>::iterator position,
...
_get_input_line_annotation(input_line_annotation<Input>& result,
                           lexy::lexeme_for<Input> line, lexy::lexeme_for<Input> newline,
