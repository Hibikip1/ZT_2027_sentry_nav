...
public:
  ...
  {
    return type() == type_production ? static_cast<pt_node_production<Reader>*>(this) :
                                       nullptr;
  }
...
struct pt_node_production : pt_node<Reader>
{
  ...
  explicit pt_node_production(production_info info) noexcept
    : pt_node<Reader>(pt_node<Reader>::type_production)
    , id(info.id)
    , child_count(0)
    , token_production(info.is_token)
...
public:
  ...
  parse_tree&& finish(lexy::lexeme<Reader> remaining_input) &&
  {
    ...
    auto node = _result._buffer.template allocate<_detail::pt_node_token<Reader>>(
        lexy::eof_token_kind, remaining_input.begin(), remaining_input.end());
  ...
  auto start_production(production_info production)
  {
    ...
    auto node = _result._buffer.template allocate<_detail::pt_node_production<Reader>>(
        production);
...
public:
  ...
  public:
    class iterator
      : public _detail::forward_iterator_base<iterator, _pt_node, _pt_node, void>
    {
    public:
...
public:
  ...
  class iterator
    : public _detail::forward_iterator_base<iterator, _value_type, _value_type, void>
  {
  public:
