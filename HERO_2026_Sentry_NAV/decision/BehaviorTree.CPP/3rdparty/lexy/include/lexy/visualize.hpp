...
OutputIt visualize_to(OutputIt out, lexy::code_point cp, visualization_options opts = {})
{
  ...
  else if(cp.is_control())
  {
    ...
    switch(c)
    {
      ...
      case '\n':
        ...
        {
          out = _detail::write_special_char(
              out, opts, [](OutputIt out) { return _detail::write_str(out, "LF"); });
        }
        else
        {
          out = _detail::write_special_char(
              out, opts, [](OutputIt out) { return _detail::write_str(out, "n"); });
        }
        break;
      case '\t':
        if(opts.tab_width > 0)
        ...
        {
          out = _detail::write_special_char(
              out, opts, [](OutputIt out) { return _detail::write_str(out, "HT"); });
        }
        else
        {
          out = _detail::write_special_char(
              out, opts, [](OutputIt out) { return _detail::write_str(out, "t"); });
        }
        break;
  ...
  else if(cp.value() == ' ')
  {
    if(opts.is_set(visualize_space))
    {
      ...
      {
        out = _detail::write_special_char(
            out, opts, [](OutputIt out) { return _detail::write_str(out, "SP"); });
      }
      else
      {
        out = _detail::write_special_char(
            out, opts, [](OutputIt out) { return _detail::write_str(out, "u0020"); });
      }
    }
    else
    {
  ...
  else if(cp.value() == '\\')
  {
    if(!opts.is_set(visualize_use_unicode))
      out = _detail::write_special_char(
          out, opts, [](OutputIt out) { return _detail::write_str(out, "\\"); });
    else
                      ...
                      [[maybe_unused]] visualization_options opts = {})
{
  ...
  else if constexpr(lexy::is_node_encoding<encoding>)
  {
    ...
    lexy::range_input<typename encoding::char_encoding, typename Reader::iterator> input(
        lexeme.begin(), lexeme.end());
    return visualize_to(out,
                        lexy::lexeme_for<decltype(input)>(input.begin(), input.end()));
  }
  else
  {
...
OutputIt visualize_to(OutputIt out, const Tree& tree, visualization_options opts = {})
{
  ...
  auto write_prefix = [opts, label, &is_last_child](OutputIt out, std::size_t cur_depth,
                                                    bool cur_is_last) {
    if(cur_depth == 0)
