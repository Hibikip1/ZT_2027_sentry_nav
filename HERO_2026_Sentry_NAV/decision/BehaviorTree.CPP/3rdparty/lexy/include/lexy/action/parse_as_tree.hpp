  ...
  public:
    ...
    void on(_pth& handler, parse_events::production_cancel ev, iterator pos)
    {
      if(--handler._depth > 0)
      {
        ...
        handler._builder->token(lexy::error_token_kind, _validate.production_begin(),
                                pos);
      }
...
struct parse_as_tree_action
{
  ...
  constexpr auto operator()(Production, const Input& input) const
  {
    ...
    return lexy::do_action<Production, result_type>(handler(*_tree, input_holder, sink),
                                                    _state, reader);
  }
};
...
{
  return parse_as_tree_action<void, Input, ErrorCallback, TokenKind, MemoryResource>(
      tree, callback)(Production{}, input);
}
template <typename Production, typename TokenKind, typename MemoryResource,
...
auto parse_as_tree(parse_tree<lexy::input_reader<Input>, TokenKind, MemoryResource>& tree,
    ...
    -> validate_result<ErrorCallback>
{
  return parse_as_tree_action<State, Input, ErrorCallback, TokenKind, MemoryResource>(
      state, tree, callback)(Production{}, input);
}
template <typename Production, typename TokenKind, typename MemoryResource,
          typename Input, typename State, typename ErrorCallback>
auto parse_as_tree(parse_tree<lexy::input_reader<Input>, TokenKind, MemoryResource>& tree,
