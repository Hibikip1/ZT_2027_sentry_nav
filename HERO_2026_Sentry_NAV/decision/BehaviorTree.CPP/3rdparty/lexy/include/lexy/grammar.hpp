...
#define LEXY_REQUIRE_BRANCH_RULE(Rule, Name)                                             \
  static_assert(lexy::_require_branch_rule<Rule>, Name " requires a branch condition."   \
                                                       " You may need to use `>>` to "   \
                                                       "specify the condition that is "  \
                                                       "used for dispatch."              \
                                                       " See "                           \
                                                       "https://lexy.foonathan.net/"     \
                                                       "learn/branching/ for more "      \
                                                       "information.")
...
struct production_info
{
  ...
  constexpr production_info(Production)
    : id(_detail::type_id<Production>())
    , name(production_name<Production>())
...
struct _sfinae_sink
{
  ...
  LEXY_FORCE_INLINE constexpr void operator()(Args&&... args)
  {
    if constexpr(!is_sink_callback_for<Sink, Args&&...>)
      ...
      // overload exists.
      static_assert(_detail::error<Production, Args...>, "missing value sink callback "
                                                         "overload for production");
    _sink(LEXY_FWD(args)...);
  }
...
public:
  ...
  constexpr return_type operator()(Args&&... args) const
  {
    ...
    else
    {
      ...
      static_assert(_detail::error<Production, Args...>, "missing value callback "
                                                         "overload for production");
    }
  }
