...
struct _list_sink
{
  ...
  constexpr auto operator()(U&& obj)
      -> decltype(LEXY_DECLVAL(C&).push_back(LEXY_FWD(obj)))
  {
...
constexpr auto collect(Callback&& callback)
{
  ...
  static_assert(std::is_void_v<typename callback_t::return_type>, "need to specify a "
...
{
  using callback_t = std::decay_t<Callback>;
  static_assert(!std::is_void_v<typename callback_t::return_type>, "cannot collect a "
                                                                   "void callback into a "
                                                                   "container");
