...
{
  static_assert(lexy::_detail::error<Item, Sep>, "list() does not support "
...
struct _lstt : rule_base
{
  ...
  LEXY_PARSER_FUNC static bool _loop(_state initial_state, TermParser& term,
                                     Context& context, Reader& reader, Sink& sink)
  {
  ...
  struct p
  {
    ...
    LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args)
    {
      ...
      // Parse the remaining items.
      if(!_loop(result ? _state::terminator : _state::recovery, term, context, reader,
                sink))
        return false;
