...
struct _del_chars
{
  ...
  constexpr void parse_one(Context& context, Reader& reader, Sink& sink)
  {
    ...
    else if constexpr(!std::is_same_v<decltype(CharClass::char_class_match_cp(
                                          char32_t())),
                                      std::false_type>)
    {
      if constexpr(lexy::is_unicode_encoding<encoding>)
      {
        static_assert(CharClass::char_class_unicode(), "cannot use this character class "
      ...
      {
        static_assert(!CharClass::char_class_unicode(), "cannot use this character class "
                                                        "with non-Unicode char encoding");
        LEXY_ASSERT(reader.peek() != encoding::eof(), "EOF should be checked before "
                                                      "calling this");
...
struct _escape : _escape_base
{
                                      ...
                                      _del_chars<Char, Reader>& cur_chars)
  {
    ...
    else if(!found)
    {
      ...
      auto err =
          lexy::error<Reader, lexy::invalid_escape_sequence>(begin, reader.position());
      context.on(_ev::error{}, err);
    }
