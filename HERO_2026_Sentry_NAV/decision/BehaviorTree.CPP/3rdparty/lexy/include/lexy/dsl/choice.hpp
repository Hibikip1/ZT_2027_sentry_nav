...
struct _chc
  ...
  // likely a bug.
  : std::conditional_t<(lexy::is_unconditional_branch_rule<R> || ...), rule_base,
                       branch_base>
{
  ...
  struct bp<Reader, lexy::_detail::index_sequence<Idx...>>
  {
        ...
        -> std::conditional_t<_any_unconditional, std::true_type, bool>
    {
      ...
      {
        LEXY_ASSERT(found_branch, "it is unconditional, but we still haven't found a "
                                  "rule?!");
        return {};
      }
      else
    ...
    LEXY_PARSER_FUNC bool finish(Context& context, Reader& reader, Args&&... args)
    {
      ...
      auto result = false;
      (void)((Idx == branch_idx ?
                  (result = r_parsers.template get<Idx>().template finish<NextParser>(
