...
public:
  ColorMapModel(
    const std::string & color_map_file, const std::string & yaml_file){
  ...
  bool World2ColorMap(double & wx, double & wy, unsigned & mx, unsigned & my){
    ...
    my=image_height_-my-1; //图像坐标系y轴向下，世界坐标系y轴向上，需要转换
    if (mx >= image_width_ || my >= image_height_) {
      std::cerr << "World coordinates out of bounds: (" << wx << ", " << wy << ") -> (" << mx << ", " << my << ")\n";
      return false;
    }
    return true;
  }
 void ReadFileAndCreateMap()
{
  ...
  cv::parallel_for_(cv::Range(0, hsv_image.rows), [&](const cv::Range& range) {
    for (int i = range.start; i < range.end; ++i) {
      ...
      for (int j = 0; j < hsv_image.cols; ++j) {
        ...
        if (((h >= 0 && h <= 10) || (h >= 156 && h <= 180)) && (s >= MIN_S) && (v >= MIN_V)) {
  ...
  cv::imwrite("/home/dji/hero2026_-sentry/src/perception/nav_cv_bridge/debug_img/res.png", res_mat);
}
  /* 提取彩色区域地图指定位置信息*/
  inline int GetColorMapInfo(unsigned int mx, unsigned int my){
    if (mx >= image_width_ || my >= image_height_) {
      std::cerr << "Pixel coordinates out of bounds: (" << mx << ", " << my << ")\n";
      return ERROR_CODE;
    }
  ...
  void PrintMAPinfo()
  {
    ...
    std::cerr << "origin_: [" << origin_[0] << ", " << origin_[1] << "]" << std::endl;
