...
// GICP 配准所需的最少点数(低于此值跳过配准,避免空点云段错误)
constexpr size_t kMinRegistrationPoints = 100;
...
void SmallGicpRelocalizationNode::performRegistration()
{
  ...
  // 加固:全局地图(先验 PCD)未加载或点云过小时跳过配准,避免 GICP 空点云 SIGSEGV
  if (
    !target_ || target_->empty() || source_->empty() ||
    static_cast<size_t>(target_->size()) < kMinRegistrationPoints ||
    static_cast<size_t>(source_->size()) < kMinRegistrationPoints) {
    RCLCPP_WARN_THROTTLE(
      this->get_logger(), *this->get_clock(), 2000,
      "Too few points for registration (target=%zu, source=%zu), skipping. "
      "Check prior PCD file.",
      target_ ? target_->size() : 0, source_->size());
    accumulated_cloud_->clear();
    return;
  }
